...
float tmv = 0.0f, startTimeJump = 0.0f;
...
std::shared_ptr<ThirdPersonCamera> TP_Camera(new ThirdPersonCamera());
...
size_t animationMayowIndex = 1, animationLeonIndex = 0;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	TP_Camera->setDistanceFromTarget(distanceFromTarget);
...
bool processInput(bool continueApplication) {
	...
	if (modelSelected == 0 && glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS){
		...
		FP_Camera->mouseMoveCamera(-0.02f, 0, deltaTime);
	...
	/*if(modelSelected == 0 && glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS && isNotJump){
		modelMatrixLeon = glm::translate(modelMatrixLeon, glm::vec3(0.0, 2.0, 0.0));
...
void applicationLoop() {
	...
	while (psi) {
		...
		if(isFPC){
			cameraMatrix = glm::translate(modelMatrixLeon,glm::vec3(0, 4.0f, 0.5f));
			//cameraMatrix = glm::rotate(cameraMatrix, glm::radians(180.0f) , glm::vec3(0.0f,1.0f,0.0f));
		}else
			cameraMatrix = glm::translate(modelMatrixLeon,glm::vec3(0, 2.0f, 0));
		...
		*******************************************/
		...
		float currHeight = terrain.getHeightTerrain(modelMatrixMayow[3][0], modelMatrixMayow[3][2]);

		modelMatrixMayow[3][1] = -GRAVITY * tmv * tmv + 3.1 * tmv + currHeight;//Gravedad
		
		tmv = currTime - startTimeJump;

		if(modelMatrixMayow[3][1] <= currHeight){
			isNotJump = true;
			modelMatrixMayow[3][1] = currHeight; 
		}
